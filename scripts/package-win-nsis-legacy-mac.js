/**
 * package-win-nsis-legacy-mac.js
 *
 * 为什么需要这个脚本：
 ...
 *   该符号不存在，导致 makensis 无法启动，Windows NSIS 安装包打包失败。
 *
 ...
 *
 * 与 package-win-portable-legacy-mac.js 的区别仅在于构建目标：
 *   nsis（安装版） vs portable（便携版）
 */
...
function log(message) {
  console.log(`[legacy-mac-nsis] ${message}`);
}

function fail(message) {
  console.error(`[legacy-mac-nsis] ${message}`);
  process.exit(1);
}
 ...
 * 构建 NSIS 安装包（与 portable 脚本的区别仅在此处）。
 */
function runElectronBuilder() {
  ...
  const result = spawnSync(builderBin, ['--win', 'nsis'], {
    stdio: 'inherit',
    env,
  });
...
async function main() {
  if (process.platform !== 'darwin') {
    fail('此命令仅用于旧版 macOS（10.13）。正常构建请使用：npm run package:win:nsis');
  }
  ...
  log('building Windows NSIS installer package...');
